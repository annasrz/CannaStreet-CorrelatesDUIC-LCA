...
## PLZ matchen, um Urbanisierungsgrad zu bestimmen
```{r match_plz}
...
### Distribution of Urbanization Level
```{r urban_dist}
...
Variables should be binary factors with levels 1 and 2 for the LCA model.
...
data <- data %>% 
  mutate(speeding_binary = factor(case_when(
    ...
    levels = c(1, 2),
    labels = c("low", "high")))
table(data$speeding_binary)
table(data$speeding_total)
```
